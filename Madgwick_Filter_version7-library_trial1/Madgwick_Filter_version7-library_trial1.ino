...
void loop() {
  ...
  //if(t_interval >= (1/filter_freq)){ //Lock a filter to a specific update interval
  init_time = micros();
  ...
  float roll_final, pitch_final, yaw_final;
...
//  Serial.println(q_est_g_lock[3]);
  ...
  //}
  // Works With Delay (which changes the rate the filter runs is adaptive)
  //delay(10);
}
